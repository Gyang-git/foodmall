    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/member/packer/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.name = data.packer.name
                this.dataForm.mobile = data.packer.mobile
                this.dataForm.gender = data.packer.gender
                this.dataForm.birthday = data.packer.birthday
                this.dataForm.workType = data.packer.workType
                this.dataForm.saveTime = data.packer.saveTime
                this.dataForm.status = data.packer.status
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/member/packer/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
